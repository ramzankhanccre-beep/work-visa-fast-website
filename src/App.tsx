  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 w-80 sm:w-96 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col h-[500px]"
          >
            {/* Header */}
            ...
            <div className="flex border-b border-slate-700 bg-slate-800/50">
              <button 
                ...
                className={`flex-1 py-2 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${mode === 'text' ? 'text-amber-400 border-b-2 border-amber-400 bg-slate-800' : 'text-slate-400 hover:text-slate-200'}`}
              >
                <MessageSquare className="w-4 h-4" /> Chat
              </button>
              <button 
                onClick={() => setMode('voice')}
                className={`flex-1 py-2 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${mode === 'voice' ? 'text-amber-400 border-b-2 border-amber-400 bg-slate-800' : 'text-slate-400 hover:text-slate-200'}`}
              >
              ...
              {mode === 'text' ? (
                <>
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${msg.role === 'user' ? 'bg-amber-500 text-slate-900 rounded-tr-none' : 'bg-slate-800 text-slate-200 border border-slate-700 rounded-tl-none'}`}>
                        {msg.text}
                      </div>
                    </div>
                  ))}
                ...
                <div className="h-full flex flex-col items-center justify-center space-y-8">
                  <div className="relative">
                    <div className={`w-32 h-32 rounded-full flex items-center justify-center ${isCalling ? 'bg-amber-500/20' : 'bg-slate-800'} transition-colors duration-500`}>
                      <div className={`w-24 h-24 rounded-full flex items-center justify-center ${isCalling ? 'bg-amber-500/40 animate-pulse' : 'bg-slate-700'}`}>
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center ${isCalling ? 'bg-amber-500' : 'bg-slate-600'}`}>
                          <Mic className={`w-8 h-8 ${isCalling ? 'text-slate-900' : 'text-slate-400'}`} />
                        </div>
                      </div>
                    </div>
                    {isCalling && (
                      <div className="absolute inset-0 rounded-full border-2 border-amber-500 animate-[ping_2s_cubic-bezier(0,0,0.2,1)_infinite]" />
                    )}
                  </div>
                  
                  <div className="text-center">
                    <h4 className="text-white font-bold text-lg mb-2">{isCalling ? 'Connected' : 'Ready to Connect'}</h4>
                    <p className="text-slate-400 text-sm max-w-[200px] mx-auto">
                      {isCalling ? 'Speak directly with our AI Immigration Consultant.' : 'Start a voice call to discuss your visa requirements.'}
                    </p>
                  </div>

                  <button
                    onClick={isCalling ? endCall : startCall}
                    className={`px-8 py-3 rounded-full font-bold flex items-center gap-2 transition-all transform hover:scale-105 ${
                      isCalling 
            ...
            {mode === 'text' && (
              <div className="p-3 bg-slate-800 border-t border-slate-700 flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-4 py-2 text-white focus:outline-none focus:border-amber-500 text-sm"
                />
                <button
                  onClick={handleSend}
                  disabled={isLoading || !input.trim()}
                  className="bg-amber-500 hover:bg-amber-400 disabled:bg-slate-700 disabled:text-slate-500 text-slate-900 p-2 rounded-xl transition-colors shrink-0"
                >
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-amber-500 hover:bg-amber-400 text-slate-900 rounded-full shadow-[0_0_20px_rgba(245,158,11,0.4)] flex items-center justify-center z-50 transition-colors"
      >
        {isOpen ? <X className="w-6 h-6" /> : <MessageCircleMore className="w-6 h-6" />}
      </motion.button>
    </>
  );
};
